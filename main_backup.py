...
GEMINI_API_URL = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent"
    """Generate answer using Gemini AI with proper formatting"""
...
2. Format with bullet points as shown
...
2. Format your response with clear sections:
   - Start with "📚 **Main Answer**" 
   - Add bullet points for key concepts
   - Include a "💡 **Key Points**" section with 2-3 important facts
   - End with "🎓 **Educational Context**" explaining the educational significance
3. Use bullet points (•) for better readability
4. Keep explanations clear and educational
5. If the context doesn't contain enough information, say so clearly
6. Make sure all information comes from the provided context
...
**Answer:**"""
        ...
        if response and response.text:
            ...
            # Ensure proper formatting if Gemini doesn't follow exactly
            if not formatted_response.startswith('📚'):
                formatted_response = f"📚 **Main Answer**\n• {formatted_response}"
...
def generate_fallback_answer(query: str, relevant_chunks: List[str]) -> str:
    """Fallback answer generation if Gemini fails"""
    primary_content = relevant_chunks[0] if relevant_chunks else ""
    
    # Create basic structured response
    sentences = [s.strip() for s in primary_content.split('.') if s.strip() and len(s.strip()) > 15]
    
    answer_parts = ["📚 **Main Answer**"]
    
    # Add main points as bullets
